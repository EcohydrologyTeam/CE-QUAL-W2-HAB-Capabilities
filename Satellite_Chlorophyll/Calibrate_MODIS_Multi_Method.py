...
def load_ukl_insitu_data():
    ...
    try:
        # First try to load from the CSV file in the current directory
        ukl_csv_path = '/Users/todd/GitHub/ecohydrology/CE-QUAL-W2-HAB-Capabilities/Satellite_Chlorophyll/UKL_insitu_chlorophyll.csv'
        raw_data = pd.read_csv(ukl_csv_path)
        
        # Convert date column
        raw_data['date'] = pd.to_datetime(raw_data['date'], errors='coerce')
        raw_data = raw_data.dropna(subset=['date', 'chlorophyll_ugL'])
        
        # Filter for satellite calibration (same logic as the original function)
        # Remove extreme values and keep reasonable range for calibration
        filtered_data = raw_data[
            (raw_data['chlorophyll_ugL'] >= 1.0) & 
            (raw_data['chlorophyll_ugL'] <= 350.0)  # Cap at 350 as you mentioned
        ].copy()
        
        print(f"Real UKL data loaded: {len(filtered_data)} records")
        print(f"Date range: {filtered_data['date'].min()} to {filtered_data['date'].max()}")
        print(f"Chlorophyll range: {filtered_data['chlorophyll_ugL'].min():.1f} - {filtered_data['chlorophyll_ugL'].max():.1f} µg/L")
        
        return filtered_data[['date', 'chlorophyll_ugL']].copy()
        
    except Exception as e:
        print(f"Warning: Could not load real UKL data from CSV ({e})")
        
        # Fallback: try the original import method
        try:
            ...
            filtered_data = filter_ukl_data_for_satellite_calibration(raw_data, min_chl=1.0, max_chl=350.0)
            return filtered_data[['date', 'chlorophyll_ugL']].copy()
            
        except Exception as e2:
            print(f"Warning: Could not load real UKL data via import ({e2}), using example data...")
